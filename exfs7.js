...
(function () {
    ...
    var COMPONENT = 'exfs_online_v7';
    var FRAME_COMPONENT = 'exfs_frame_v7';

    if (window.__exfs_v7_loaded) return;
    window.__exfs_v7_loaded = true;

    function diag(text) {
        console.log('EX-FS DEBUG', text);
        try { Lampa.Noty.show('EX-FS: ' + text); } catch (e) {}
    }
    ...
    function proxied(base, target) {
        // nb557 worker expects the full target URL directly after "/"
        return base + target;
    }

    function request(url, oncomplete, onerror, post, extra) {
        var options = extra || {};
        var timeout = options.timeout || 18000;
        ...
        // Android can use Lampa's native HTTP directly.
        if (isAndroid()) {
            var nativeNetwork = new Lampa.Reguest();
            nativeNetwork.timeout(timeout);

            nativeNetwork['native'](
                url,
                function (data) {
                    if (oncomplete) oncomplete(normalize(data));
                },
                function (a, b) {
                    if (onerror) onerror(a, b, nativeNetwork);
                },
                post || false,
                {
                    dataType: 'text',
        ...
        /*
         * Browser Lampa:
         * use the same public proxy layer used by current Online MOD.
         * Try all three endpoints automatically.
         */
        var index = 0;
        var lastNetwork = null;

        function attempt() {
            if (index >= PUBLIC_PROXIES.length) {
                if (onerror) onerror('proxy_failed', 'Усі проксі недоступні', lastNetwork);
                return;
            }

            var base = PUBLIC_PROXIES[index++];
            var network = new Lampa.Reguest();
            lastNetwork = network;
            network.timeout(timeout);

            var target = proxied(base, url);

            console.log('EX-FS proxy try', base, url);
            ...
            network.silent(
                ...
                function (data) {
                    ...
                    // Reject obvious proxy error pages and try the next endpoint.
                    if (
                        !data ||
                        /^Malformed URL/i.test(data) ||
                        /^Internal Server Error/i.test(data) ||
        ...
        return {
            clear: function () {
                try {
                    if (lastNetwork) lastNetwork.clear();
                } catch (e) {}
            }
        };
    }
    ...
    function searchOnce(query, year, done, fail) {
        ...
        request(
            ...
            function (html) {
                ...
                diag('5/6 відповідь EX-FS, знайдено: ' + items.length);
    ...
    function parseMedia(pageUrl, html) {
        ...
        Array.prototype.forEach.call(root.querySelectorAll('iframe[src],iframe[data-src],iframe[data-url]'), function (el) {
            addFrame(
    ...
    function component(object) {
        ...
        this.search = function () {
            ...
            searchMovie(
                ...
                function (a, b, req) {
                    var msg = 'EX-FS: помилка мережі/проксі';
        ...
        this.openPage = function (result) {
            ...
            request(
                ...
                function (html) {
                    ...
                    diag('6/6 сторінка: HLS/MP4=' + media.direct.length + ', iframe=' + media.frames.length);
    ...
    function startPlugin() {
        if (window.__exfs_v7_started) return;
        ...
        try {
            ...
            // Mark as started ONLY after successful initialization.
            window.__exfs_v7_started = true;

            console.log('EX-FS', 'v' + VERSION + ' started');
            diag('1/6 v' + VERSION + ' завантажено • proxy fallback');
        } catch (e) {
            ...
            // Do not lock the plugin in a broken "started" state.
    ...
    function bootstrap() {
        ...
        if (Lampa.Listener && Lampa.Listener.follow) {
            ...
            Lampa.Listener.follow('app', function (e) {
                if (!fired && e.type === 'ready') {
                    fired = true;
                    startPlugin();
                }
            });

            // Fallback for builds where appready is already set
            // but the plugin missed the "ready" event.
