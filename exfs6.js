...
(function () {
    ...
    var BROWSER_PROXY = 'https://api.allorigins.win/raw?url=';
    var COMPONENT = 'exfs_online_v6';
    var FRAME_COMPONENT = 'exfs_frame_v6';

    if (window.__exfs_v6_loaded) return;
    window.__exfs_v6_loaded = true;
    ...
    function browserProxyUrl(url) {
        return BROWSER_PROXY + encodeURIComponent(url);
    }

    function request(url, oncomplete, onerror, post, extra) {
        var network = new Lampa.Reguest();
        var options = extra || {};
        var android = isAndroidNative();
        var target = url;
        var sendPost = post || false;

        network.timeout(options.timeout || 20000);

        /*
         * Android: use Lampa native HTTP directly.
         * Browser/Opera/Linux: EX-FS does not permit us to read its HTML
         * cross-origin, so GET HTML is read through AllOrigins.
         *
         * Browser mode intentionally uses GET only. The EX-FS search URL
         * already contains the search query, so POST is unnecessary there.
         */
        if (!android) {
            target = browserProxyUrl(url);
            sendPost = false;
        }
    ...
    function searchOnce(query, year, done, fail) {
        ...
        request(
            ...
            function (html) {
                ...
                if (!isAndroidNative()) {
                    // Browser mode cannot proxy a POST reliably here.
                    done([]);
                    return;
                }
    ...
    function startPlugin() {
        if (window.__exfs_v6_started) return;
        ...
        try {
            ...
            window.__exfs_v6_started = true;

            console.log('EX-FS', 'v' + VERSION + ' started');
            diag('1/6 v' + VERSION + ' завантажено • ' + (isAndroidNative() ? 'Android native' : 'Browser proxy'));
        } catch (e) {
